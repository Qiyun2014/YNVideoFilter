...
- (void)filterGroup:(GPUImageFilterGroup *)filterGroup
    ...
    /* 如果3秒之后，渲染进度还是没有，就添加失败 */
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//        
//        if (movieFile.progress <= 0) {
//            
//            [timer invalidate];
//            [movieWriter cancelRecording];
//            if (filter_finished) filter_finished(nil);
...
+ (void)videoFilterSampleBufferRefWithUrl:(NSString *)videoUrl completed:(void (^) (void))completed{
    ...
    NSNumber *size = [[YNFileManager shareInstanceManager] getFileSizeOfFilePath:videoUrl];
    BOOL haveFile = [size floatValue] - pow(10, 5);
    ...
    CMTimeRange timeRange = CMTimeRangeMake(CMTimeMakeWithSeconds((haveFile?0:0.2), NSEC_PER_SEC), CMTimeMakeWithSeconds(asset.duration.value / asset.duration.timescale - (haveFile?0:0.4), NSEC_PER_SEC));
